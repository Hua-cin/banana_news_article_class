...
def main():
    ...
    sample_word_cut_dict = func_jieba(sample_text, stopword_list)
    print("sample")
    ...
    no_related_word_cut_dict = func_jieba(no_related_text, stopword_list)
    ...
    related_word_cut_dict = func_jieba(related_text, stopword_list)
    ...
    for i, count in enumerate(countlist):
        ...
        cd ={}
        for word, score in sorted_words[1:100]:
    ...
    h.iloc[2:, :] = h.iloc[2:, :] * 1
    # h.iloc[:2, :10] = h.iloc[:2, :10] /1.8


    # KMeans 演算法
    kmeans_fit = cluster.KMeans(n_clusters=2).fit(h)
    ...
    # 使用pandas 將資料轉為csv檔
    h.to_csv('./dd.csv', index=0, encoding="utf_8_sig")

def func_jieba(text, stopword_list):
    '''
    :param text:
